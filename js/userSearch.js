...
async function searchUser(url, methodType, callback){
 $.ajax({
    url : url,
    headers: {
        "X-API-Key": apiKey
...
document.getElementById("buttonSearch").addEventListener("click", function(){
    ...
    if(platformType){
        ...
        userNameSearch = updatedUsername;
    }
    ...
    searchUser(SearchURL, "GET", function(respJson){
        ...
        var userIDTrue = respJson.Response[0].membershipId;

        var userIDURL = "https://www.bungie.net/Platform/Destiny2/"+platformType+"/Profile/"+userIDTrue+"/?components=100";
